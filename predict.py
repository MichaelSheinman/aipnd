...
parser.add_argument('--hidden_units', type=int, default=512, help='units the loaded model has')

args, _ = parser.parse_known_args()

def loading_checkpoint(checkpoint, arch, hidden_units):
    
    # Credit to Kyu Lee for the way to turn gpu tenors to cpu 
    checkpoint_state = torch.load(checkpoint, map_location = lambda storage, loc: storage)

    class_to_idx = checkpoint_state['class_to_idx']

    # Create from a pre-trained model: added arch & hidden_units per review 
    model, optimizer, criterion = create_model(hidden_units, class_to_idx, arch)
...
def predict(image, checkpoint, topk, labels, arch, hidden_units, gpu=False):
    model, optimizer, criterion = loading_checkpoint(checkpoint, arch, hidden_units)
...
if args.image and args.checkpoint:
    probs, classes = predict(args.image, args.checkpoint, args.top_k, args.labels, args.arch, args.hidden_units ,args.gpu)
